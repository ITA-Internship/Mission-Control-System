...
from drones.serializers import DroneListSerializer, DroneSerializer, DroneUpdateSerializer, DroneModelSerializer, \
...
drone_get_schema = description_schema(
    summary="List drones",
    description="Retrieves a paginated and filtered list of all drones in the system.\n\n"
                "By default list displays only drones with active statuses. "
                "Choose one of the inactive statuses during filtration to get drones with inactive status.",
...
drone_data_import_schema = description_schema(
    ...
    description=(
        ...
        "Processes the file record row by row, validates data and return error logs if any. "
    ...
    responses={
        status.HTTP_200_OK: OpenApiResponse(
            description="Import processing completed."
        ),
    },
